...
RUN apt-get update \
    ...
    && apt-get update \
    && apt-get install -y nginx php7.1-fpm php7.1-cli php7.1-dev \
       ...
       php7.1-ldap \
       php7.1-msgpack php7.1-igbinary php7.1-redis \
       php7.1-memcached php7.1-pcov php7.1-xdebug \
    ...
    && sed -i 's/^;daemonize.*$/daemonize = no/g' /etc/php/7.1/fpm/php-fpm.conf \
    && sed -i 's@^error_log.*$@error_log = /proc/self/fd/2@g' /etc/php/7.1/fpm/php-fpm.conf
...
COPY php/php.ini /etc/php/7.1/fpm/conf.d/99-www-data.ini
